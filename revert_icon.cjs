...
const targetLayout = `                            {/* Desktop Layout */}
                            <div className="hidden lg:grid w-full lg:grid-cols-[auto_1fr_1fr_1fr] gap-3 lg:gap-4 items-stretch">
                              <div className="flex items-center justify-center pr-0 w-[200px] xl:w-[240px]">
                                <img
                                  src="/jagoan.png"
                                  className="w-full h-auto max-h-[200px] xl:max-h-[240px] object-contain object-center drop-shadow-md"
                                  alt="Jagoan Advanta"
                                />
                              </div>
                              {renderedCards}
                            ...
                            <div className="w-full flex flex-col gap-3 sm:gap-4 lg:hidden">
                              <div className="w-full flex flex-row gap-2 sm:gap-3 items-stretch">
                                <div className="flex items-center justify-center shrink-0 w-[140px] sm:w-[170px] md:w-[210px]">
                                  <img
                                    src="/jagoan.png"
                                    className="w-full h-auto max-h-[180px] sm:max-h-[210px] md:max-h-[240px] object-contain object-center drop-shadow-md"
                                    alt="Jagoan Advanta"
                                  />
                                </div>`;
...
code = code.replace(/className=\{\`rounded-\[32px\] h-full/g, 'className={`rounded-[32px]');
code = code.replace(/\[\&>button\]:h-full \[\&>button\]:flex-1/g, 'flex-1 min-w-0 flex flex-col');

fs.writeFileSync('src/App.tsx', code);
console.log("Icon layout reverted!");
